...
RATING_DEPTH = -1    # how far ahead to check a move's rating, -1 to disable
...
class Robot:
    ...
    def _move(self, game):
        ...
        if ('spawn' not in rg.loc_types(self.location)
                or not self._check_move_valid(game, move)):

            around = rg.locs_around(self.location,
    ...
    def _attack(self, game):
        ...
        for loc in all_around:
            ...
            if (loc in state['BOGY']):
                ...
                for check in check_locs:
                    if (loc[0] == self.location[0]+check[0]
                            and loc[1] == self.location[1]+check[1]):
        ...
        if (closest_loc != ()):
            ...
            if (self._check_move_valid(game, move)):
                ...
                # If the distance is close enough then attack,
                # bogy might move there
                if closest_dist == 2:
                    return ['attack', move]
    ...
    def _check_move_valid(self, game, loc):
        ...
        if ('spawn' in rg.loc_types(loc)
                and 'spawn' not in rg.loc_types(self.location)):
            return False
